...
export function AdjustingPage() {
  ...
  const { data: entries, isLoading } = useAdjustingEntries(startDate, endDate);
  const { data: accounts } = useAccounts();
  const { mutate: createEntry, isPending } = useCreateAdjustingEntry();
  const { mutate: deleteEntry } = useDeleteAdjustingEntry();
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="print-report-root soft-card shadow-sm overflow-hidden border border-border/40">
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow><TableCell colSpan={6} className="text-center py-12 text-muted-foreground">Loading adjusting entries...</TableCell></TableRow>
            ) : entries?.length === 0 ? (
              <TableRow><TableCell colSpan={6} className="text-center py-12 text-muted-foreground">No adjusting entries found for this period.</TableCell></TableRow>
            ) : (
              entries?.map((entry) => (
                  ...
                  {entry.details.map((d, i) => (
                    <TableRow key={`${entry.id}-${i}`} className="hover:bg-slate-50/50 border-0 group">
                      ...
                      <TableCell className="pt-3 border-0 align-top w-12 pr-4">
                        {i === 0 && (
                          <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => setDeleteData({ id: entry.id, description: entry.description || 'Adjusting Entry' })}>
      ...
      <AlertDialog open={!!deleteData} onOpenChange={(open) => !open && setDeleteData(null)}>
        <AlertDialogContent className="rounded-2xl border-border/40 shadow-xl">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Adjusting Entry</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete the entry <span className="font-semibold text-slate-900">"{deleteData?.description}"</span>?
